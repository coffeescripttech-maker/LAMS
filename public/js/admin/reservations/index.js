...
const state = {
    entity: {
        name: "reservation",
        attributes: ["faculty_name", "schedule"],
        form: [
            {
                class: "form-control",
    ...
    ask: async () => {
        ...
        const faculties = await fetch.ask("../api/users/list", {
            key: "faculty",
        });
        state.faculties = faculties;
        state.entity.form[1].options = faculties.map((faculty) => {
        ...
        state.entity.form[0].options = await fetch
            .ask("../api/schedules/list", { booked: false })
            .then((schedules) => {
    ...
    writter: async (model, i) => {
        ...
        $("<td>", {
            class: "text-center",
            html: `<span class="${await state.handleStatus(
                model.status
            )} text-capitalize">${
                model.status === undefined ? "pending" : model.status
            }</span>`,
        }).appendTo(tr);
    ...
    onShow: async (i) => {
        ...
        $("#title").html("Edit");
        state.activeIndex = i;
        $("#schedule_id").val(state.models[i].schedule_id);
