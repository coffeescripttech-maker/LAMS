...
def preprocess_image(img):
    ...
    _, binary = cv2.threshold(blurred, 90, 255, cv2.THRESH_BINARY_INV)  # Lowered threshold to 30
...
def match_fingerprints(minutiae1, minutiae2, distance_threshold=18):
    points1 = np.array([m[1] for m in minutiae1])
    points2 = np.array([m[1] for m in minutiae2])
    ...
    return matches / max(len(points1), len(points2))
